 ...
 * Contains: Config, Phase tracker, Chat, File explorer, File viewer.
 */
import React, { useCallback, useEffect, useRef, useState } from 'react';
import {
  X, Settings2, MessageSquare, FolderOpen,
  Send, Loader2, FileText, RefreshCw, Save, Edit3, Trash2,
...
const MarkdownBlock: React.FC<{ content: string }> = ({ content }) => {
  // Simple markdown to HTML (handles headers, bold, italic, code blocks, lists)
...
const PhaseTracker: React.FC<{
  ...
  <div className={cn('flex items-center gap-1 px-3 py-2 border-b', isDark ? 'border-white/5' : 'border-gray-200')}>
    {PHASE_ORDER.map((phase, i) => {
      const done = phasesStatus?.[phase]?.completed;
      const active = currentPhase === phase;
      return (
        <React.Fragment key={phase}>
          {i > 0 && <div className={cn('h-px flex-1 transition-colors', done ? 'bg-emerald-500/60' : (isDark ? 'bg-white/10' : 'bg-gray-200'))} />}
          <div
            className={cn(
          ...
          >
            {done ? <CheckCircle2 className="size-3" /> : active ? <Loader2 className="size-3 animate-spin" /> : <Circle className="size-3" />}
...
const FileExplorer: React.FC<{
  ...
  <div className="flex flex-col gap-0.5 overflow-y-auto">
    <div className="flex items-center justify-between px-3 py-1.5">
      <span className={cn('text-[10px] font-semibold uppercase tracking-wider', isDark ? 'text-white/40' : 'text-gray-500')}>Archivos</span>
    ...
    {files.length === 0 ? (
      ...
      files.map((f) => (
        ...
        >
          ...
          <span className={cn('ml-auto text-[9px]', isDark ? 'text-white/20' : 'text-gray-400')}>{(f.size / 1024).toFixed(1)}k</span>
        </button>
      ))
    )}
  </div>
...
export const SDDPanel: React.FC<SDDPanelProps> = ({ open, onClose, isDarkTheme = true, onImportDiagram }) => {
  const {
  ...
  const [activeTab, setActiveTab] = useState<'chat' | 'files' | 'config'>('chat');
  ...
  // Auto-import diagram when generated
  useEffect(() => {
  ...
  const handleSaveFile = useCallback(() => {
    if (selectedFile && editBuffer !== null) {
      writeFile(selectedFile, editBuffer);
      setFileContent(editBuffer);
      setIsEditing(false);
    }
  }, [selectedFile, editBuffer, writeFile]);
  ...
  const t = {
    ...
    userBubble: isDarkTheme ? 'bg-blue-600/80 text-white' : 'bg-blue-600 text-white',
    assistantBubble: isDarkTheme ? 'bg-white/[0.06] text-white/90' : 'bg-gray-100 text-gray-800',
    systemBubble: isDarkTheme ? 'bg-red-500/10 text-red-300 border border-red-500/20' : 'bg-red-50 text-red-700 border border-red-200',
    ...
    textarea: isDarkTheme
      ...
      : 'border-gray-300 bg-white text-gray-900 placeholder:text-gray-400 focus:border-blue-500',
    fileSelected: isDarkTheme ? 'bg-blue-500/15 text-blue-300' : 'bg-blue-50 text-blue-700',
    fileDefault: isDarkTheme ? 'text-white/60 hover:bg-white/[0.04] hover:text-white/80' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-800',
    fileSize: isDarkTheme ? 'text-white/20' : 'text-gray-400',
    fileLabel: isDarkTheme ? 'text-white/40' : 'text-gray-500',
    fileRefresh: isDarkTheme ? 'text-white/30 hover:text-white/60' : 'text-gray-400 hover:text-gray-600',
    fileEmpty: isDarkTheme ? 'text-white/20' : 'text-gray-400',
    fileName: isDarkTheme ? 'text-white/60' : 'text-gray-600',
    editBtn: isDarkTheme ? 'text-white/30 hover:bg-white/[0.06] hover:text-white/60' : 'text-gray-400 hover:bg-gray-100 hover:text-gray-600',
    editArea: isDarkTheme ? 'text-white/80' : 'text-gray-800',
    recentDir: isDarkTheme ? 'bg-white/[0.04] text-white/40 hover:bg-white/[0.08] hover:text-white/60' : 'bg-gray-100 text-gray-500 hover:bg-gray-200 hover:text-gray-700',
    headerBtn: isDarkTheme ? 'text-white/40 hover:bg-white/[0.06] hover:text-white/70' : 'text-gray-400 hover:bg-gray-100 hover:text-gray-600',
  };
  ...
  return (
    <div className={cn('flex h-full w-[380px] shrink-0 flex-col border-r animate-in slide-in-from-left-4 duration-200', t.panel)}>
      ...
      {showConfig && (
        <div className={cn('border-b px-3 py-3 space-y-2.5 animate-in slide-in-from-top-2 duration-150', t.border, t.configBg)}>
          ...
          <button
            ...
            className="w-full rounded-md bg-blue-600/80 py-1.5 text-[11px] font-semibold hover:bg-blue-600 transition-colors"
          >
      ...
      {/* ---- Phase Tracker ---- */}
      <PhaseTracker phasesStatus={phasesStatus} isDark={isDarkTheme} currentPhase={isProcessing ? undefined : phasesStatus ? PHASE_ORDER.find((p) => !phasesStatus[p]?.completed) : undefined} />
      ...
      {activeTab === 'files' && (
        <div className="flex min-h-0 flex-1 flex-col">
          <div className="shrink-0">
            <FileExplorer files={files} onFileSelect={handleFileSelect} onRefresh={refreshFiles} selectedFile={selectedFile} isDark={isDarkTheme} />
          </div>
          {selectedFile && (
            <div className={cn('flex min-h-0 flex-1 flex-col border-t', t.border)}>
              {/* File header */}
              <div className={cn('flex items-center justify-between px-3 py-1.5 border-b', t.border)}>
                <span className={cn('text-[11px] font-semibold truncate', t.fileName)}>{selectedFile}</span>
                <div className="flex items-center gap-1">
                  {isEditing ? (
                    <button onClick={handleSaveFile} className="rounded p-1 text-emerald-400 hover:bg-emerald-500/10" title="Guardar">
                      <Save className="size-3" />
                    </button>
                  ) : (
                    <button onClick={() => { setIsEditing(true); setEditBuffer(fileContent || ''); }} className={cn('rounded p-1', t.editBtn)} title="Editar">
                      <Edit3 className="size-3" />
                    </button>
                  )}
                  <button onClick={() => { setSelectedFile(null); setFileContent(null); setIsEditing(false); }} className={cn('rounded p-1', t.editBtn)}>
                    <X className="size-3" />
                  </button>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-3 py-2">
                {fileContent === null ? (
                  ...
                  <textarea
                    ...
                    className={cn('w-full h-full resize-none bg-transparent text-xs font-mono focus:outline-none', t.editArea)}
                  />
                ) : (
